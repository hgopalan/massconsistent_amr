...
- **3D Meteorological Ingestion (NetCDF)** — Horizontal, terrain-aware vertical, and temporal interpolation of 3D NWP model outputs (e.g. WRF, GFS) into the solver grid. Includes anisotropic Inverse Distance Weighting (IDW) with vertical scaling parameter :math:`\gamma \gg 1` to preserve vertical atmospheric profiles.
...
- **Gaussian puff dispersion** — Passive pollutant transport with enhanced physics:
  - **Adaptive Time-Stepping** — Dynamic scaling of :math:`\Delta t` based on cell-local grid size and maximum wind velocity to ensure Courant–Friedrichs–Lewy (CFL) stability.
