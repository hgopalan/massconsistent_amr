...
.. list-table::
   ...
   * - File
     - Purpose
   * - ``inputs.i``
     - Original Gaussian Hill (baseline, no turbulence)
   * - ``inputs_mann_box.i``
     - **NEW: Gaussian Hill with Mann Box turbulence**
   * - ``inputs_synthetic_turbulence.i``
     - Existing IEC/Von Kármán/Kaimal configuration
   * - ``terrain.csv``
     - Gaussian Hill terrain elevation data (11×11 grid)
   * - ``test_mann_box_inputs.py``
     - **NEW: Test suite for Mann Box configuration**
   * - ``test_case1.py``
     - Existing Python test case
