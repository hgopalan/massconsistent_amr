...
class CoupledWindFireSimulation:
    ...
    def run(self,
        ...
        print(f"Wind update interval: {wind_update_interval}")
        print()
        ...
        try:
            while True:
                ...
                # Print progress
                status_str = f"Step {step_count + 1:4d}: t={self.fire_time:8.1f}s, burned={burned_fraction:5.1f}%"
                if result['wind_solved']:
                    status_str += f", wind_iters={result.get('wind_iters', 0):3d}"
                if result['heat_source_added']:
                    status_str += ", heat_src"
                print(status_str)
                ...
                # Write plotfiles at specified interval
                if plot_interval is not None and self.fire_time >= next_plot_time:
                    plotfile_fire = f"plt_fire_coupled_{plot_num:04d}"
                    plotfile_wind = f"plt_wind_coupled_{plot_num:04d}"
                    try:
                        self.fire.write_plotfile(plotfile_fire)
                        self.wind.write_plotfile(plotfile_wind)
                        print(f"  → Wrote plotfiles: {plotfile_fire}, {plotfile_wind}")
                    except Exception as e:
                        print(f"  → Warning: Failed to write plotfiles: {e}")
        ...
        if 'ros' in fire_state:
            print(f"  Max ROS: {fire_state['ros'].max():.2f} m/s")
        
        if 'intensity' in fire_state:
            print(f"  Max intensity: {fire_state['intensity'].max():.0f} kW/m")
        
        print("\n✓ Coupled simulation completed successfully!")
        
        return {'success': True, 'final_time': self.fire_time, 'steps': step_count}
    
    def finalize(self):
        """
        Finalize and cleanup both solvers.
        ...
        try:
            ...
            print(f"Warning: Wind solver finalization failed: {e}")
            success = False
        ...
        try:
            ...
            print(f"Warning: Fire solver finalization failed: {e}")
            success = False
    ...
    def get_status(self) -> Dict:
        """
        Get current coupled simulation status.
        
        Returns:
            dict: Dictionary with:
                - 'fire_time': Current fire time (seconds)
                - 'fire_step': Current fire step number
                - 'coupled_steps': Total coupled steps executed
                - 'coupling_mode': Coupling mode ('one_way' or 'two_way')
                - 'domain_compatible': Whether domains are compatible
        """
        return {
            'fire_time': self.fire_time,
            'fire_step': self.step_count,
