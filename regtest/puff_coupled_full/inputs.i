# Gaussian Puff Model - Full Coupled Test
# Tests terrain + buildings + canopy all together
# Domain: 400m x 400m x 150m
# Terrain: Gaussian hill
# Buildings: Urban area with 3 buildings
# Canopy: Forest canopy 15m tall
# Source: upwind at 20m height
# Wind: 10 m/s from west

# Enable puff model
enable_puff = true

# Source location and emission
source_x  = 100.0      # Source x-coordinate [m] (upwind)
source_y  = 200.0      # Source y-coordinate [m]
source_z  = 35.0       # Source height above sea level [m]
emission_rate = 1.0    # Emission rate [units/s]
emission_duration = 60.0  # Duration of emission [s]

# Diffusivity parameters
K_h = 1.0              # Horizontal diffusivity [m²/s]
K_v = 0.5              # Vertical diffusivity [m²/s]

# Initial puff size
sigma_y0 = 1.0         # Initial lateral width [m]
sigma_z0 = 1.0         # Initial vertical height [m]

# Wind field (uniform for this test)
U_wind = 10.0          # x-component of wind [m/s]
V_wind = 0.0           # y-component of wind [m/s]
W_wind = 0.0           # z-component of wind [m/s]

# ============================================================================
# Terrain awareness
# ============================================================================
terrain_file = terrain.csv           # Gaussian hill terrain
enable_terrain_reflection = true    # Enable ground reflection
use_image_source = true              # Use image source method

# ============================================================================
# Building awareness
# ============================================================================
building_file = buildings.csv        # Multiple buildings
enable_building_masking = true       # Enable collision detection
enable_wake_diffusivity = true       # Enable wake enhancement

# Wake model parameters (Röckle 1990)
wake_c1 = 0.9                        # Cavity length coefficient
wake_c2 = 0.3                        # Wake deficit coefficient
wake_separation_length = 3.0         # Wake extends to 3*H downwind

# Wake diffusivity enhancement factors
wake_enhancement_cavity = 3.0        # 3x enhancement in cavity
wake_enhancement_far = 1.5           # 1.5x enhancement in far wake

# ============================================================================
# Canopy effects
# ============================================================================
enable_canopy_effects = true         # Enable canopy effects
canopy_height = 15.0                 # Forest canopy height [m]
frontal_area_index = 0.30            # Frontal area index
canopy_enhancement_factor = 3.0      # Vertical diffusivity enhancement
canopy_sheltering_factor = 0.7       # Horizontal diffusivity reduction

# Canopy deposition (particles/smoke)
enable_canopy_deposition = true      # Enable dry deposition
deposition_velocity = 0.01           # Deposition velocity [m/s]

# ============================================================================
# Domain and discretization
# ============================================================================
xmin = 0.0             # Domain min x [m]
xmax = 400.0           # Domain max x [m]
ymin = 0.0             # Domain min y [m]
ymax = 400.0           # Domain max y [m]
zmin = 0.0             # Domain min z [m]
zmax = 150.0           # Domain max z [m]

# Concentration grid resolution
dx = 20.0              # Grid spacing x [m]
dy = 20.0              # Grid spacing y [m]
dz = 10.0              # Grid spacing z [m]

# Time stepping
dt_puff = 0.5          # Time step [s]
n_steps_puff = 120     # Number of time steps (total time = 60 s)
output_freq_puff = 24  # Write concentration every 24 steps (every 12 s)

# Output file
puff_output = puff_concentration_coupled.csv  # Output file prefix
